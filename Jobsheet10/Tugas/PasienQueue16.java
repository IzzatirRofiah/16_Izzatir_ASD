public class PasienQueue16 {
    Pasien16[] antrian;
    int front;
    int rear;
    int size;
    int max;
    public PasienQueue16(int n) {
        max = n;
        antrian = new Pasien16[max];
        size = 0;
        front = rear = -1;
    }
    ...
    public void enqueue(Pasien16 pasien) {
        if (isFull()) {
    ...
    public Pasien16 dequeue() {
        Pasien16 pasien = null;
        if (isEmpty()) {
    ...
    public void print() {
        if (isEmpty()) {
            ...
            int i = front;
            do {
                System.out.println("Nama: " + antrian[i].nama + ", No. ID: " + antrian[i].noID + ", Jenis Kelamin: "
                        + antrian[i].jenisKelamin + ", Umur: " + antrian[i].umur);
                i = (i + 1) % max;
            } while (i != (rear + 1) % max);
        }
    }
